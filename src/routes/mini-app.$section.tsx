...
function GrowthIntelligence({ auth, data, actions, reload, runAction, actionBusy }: any) {
  ...
  return <div className="min-w-0 space-y-3 overflow-x-hidden pb-[calc(2rem+env(safe-area-inset-bottom))]">
    ...
    {(view?.destinations ?? []).map((row: any) => <button type="button" key={row.id} onClick={() => setDetailId(detailId === row.id ? null : row.id)} className="block w-full min-w-0 border border-border bg-card p-3 text-left">
      ...
      <div className="mt-3 h-44 min-w-0" onClick={(event) => event.stopPropagation()}><ResponsiveContainer width="100%" height="100%"><LineChart data={row.chart}><CartesianGrid strokeDasharray="3 3" opacity={0.2}/><XAxis dataKey="bucket" tickFormatter={(v) => new Date(v).toLocaleDateString(undefined,{month:"short",day:"numeric"})} minTickGap={24} tick={{fontSize:10}}/><YAxis width={42} tick={{fontSize:10}}/><Tooltip labelFormatter={(v) => new Date(v).toLocaleString()}/><Line type="linear" dataKey="memberCount" name="Members" stroke="#22d3ee" connectNulls={false} dot={row.chart?.length < 12}/><Line type="linear" dataKey="netGrowth" name="Net Growth" stroke="#22c55e" connectNulls={false} dot={false}/></LineChart></ResponsiveContainer></div>
      <div className="mt-2 space-y-0.5 text-[11px] text-muted-foreground"><p>Live tracking: {row.status === "ACTIVE" ? "Active" : row.status}</p><p>Historical backfill: {row.coverage?.lastError ? row.coverage.lastError : row.coverage?.backfillComplete ? "Complete to Telegram-available history" : "In progress"}</p>{row.coverage?.oldestEventAt ? <p>Oldest collected: {new Date(row.coverage.oldestEventAt).toLocaleString()}</p> : null}<p>Last sync: {row.coverage?.lastSync ? new Date(row.coverage.lastSync).toLocaleString() : "Pending"}</p></div>
