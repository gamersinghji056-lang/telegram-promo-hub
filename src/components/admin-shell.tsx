        ...
        <nav className="flex gap-1 overflow-x-auto px-3 pb-3 lg:block lg:space-y-1">
          {items.map(([slug, label, Icon]) => (
            <Link
              ...
              to="/admin/$section"
              params={{ section: slug }}
              className={`flex shrink-0 items-center gap-3 rounded-md px-3 py-2.5 text-sm ${active === slug ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}
