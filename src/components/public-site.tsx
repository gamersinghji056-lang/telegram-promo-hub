...
import {
  ...
  Compass, CreditCard, ExternalLink, FolderCheck, Gauge, Layers3, Menu, MessageCircle,
...
const homeFaqs = [
  ...
  ["Is MARK available now?", "No. MARK is not live yet. Start, Try, Open or Use actions show the Coming Soon experience."],
...
function ProductFrame({ kind = "promotion", mode = "overview" }: { kind?: "promotion" | "mark"; mode?: "overview" | "campaigns" | "audience" | "sessions" | "analytics" | "growth" | "activity" }) {
  if (kind === "mark") {
    return <div className="product-frame mark" aria-label="MARK concept preview">
      <div className="product-frame-bar"><i /><i /><i /><span>MARK concept - coming soon</span></div>
      <div className="mark-concept"><div className="mark-core"><BrainCircuit /><strong>MARK</strong></div><div className="mark-node n1">Knowledge</div><div className="mark-node n2">Instructions</div><div className="mark-node n3">Context</div><div className="mark-node n4">Memory</div><p>Planned intelligence workspace</p></div>
    </div>;
  }
...
function HomePage() {
  return <MarketingShell>{({ showChooser, showMark, config }) => <>
    <section className="public-hero"><div className="public-orb orb-a" /><div className="public-orb orb-b" /><div className="public-container public-hero-grid">
      ...
      <div className="hero-composition"><ProductFrame mode="overview" /><div className="hero-mark-card"><div className="public-mark-symbol"><BrainCircuit /></div><strong>MARK</strong><span>Intelligence built around your business.</span><button onClick={showMark}>Preview Coming Soon</button></div></div>
    </div></section>
    <section className="public-section" id="products"><div className="public-container"><SectionHeading eyebrow="OUR PRODUCTS" title="Two products with separate product boundaries." copy="Telegram Promotion is live today. MARK is a future intelligent assistant and does not share the live Promotion Mini App." />
